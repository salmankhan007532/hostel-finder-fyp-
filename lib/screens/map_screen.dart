...
import 'package:hostel_finder_noor_rehman/screens/hostel_details_screen.dart';
import 'package:hostel_finder_noor_rehman/screens/search_screen.dart';
...
class _MapScreenState extends State<MapScreen> {
  ...
  Future getAllHostels() async {
    DatabaseReference hostelRef =
        FirebaseDatabase.instance.ref().child('hostels');
    var snapshot = await hostelRef.once();
    var hostelSnapshot = snapshot.snapshot.value;

    if (hostelSnapshot == null) {
      print('***********************************');
      print('No hostels');
      return;
    }

    var hostelFullMap = Map<String, dynamic>.from(hostelSnapshot as Map);

    for (var hostelMap in hostelFullMap.values) {
      markers.add(Marker(
        markerId: MarkerId('${hostelMap['hostelId']}'),
        position: LatLng(hostelMap['latitude'], hostelMap['longitude']),
        icon: await BitmapDescriptor.fromAssetImage(
            ImageConfiguration.empty, 'assets/icon/marker.png'),
        infoWindow: InfoWindow(
            title: '${hostelMap['hostelName']}',
            onTap: () {
              Hostel hostel = Hostel.fromMap(Map<String, dynamic>.from(hostelMap));
              //Fluttertoast.showToast(msg: hostel.ownerName);
              Navigator.of(context).push(MaterialPageRoute(builder: (context) {
                return HostelDetailsScreen(hostel: hostel);
              }));
            }),
      ));
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: (){
            Navigator.of(context).push(MaterialPageRoute(builder: (context){
              return SearchScreen(hostels:hostels);
            }));
